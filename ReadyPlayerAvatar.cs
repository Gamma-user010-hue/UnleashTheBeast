...
public class ReadyPlayerAvatar : MonoBehaviour
{
    ...
    private async void InitializeAvatar(){
        ...
        if (success) {
            ...
            transform.position = new Vector3(5.3f, 0f, -18.75f);
            ...
            PoseBlockDetector detector = FindFirstObjectByType<PoseBlockDetector>();

            if (detector != null)
            {
                detector.InitializeForAvatar(this.transform);
            }
            else
            {
                Debug.LogWarning("PoseBlockDetector not found in scene!");
            }
